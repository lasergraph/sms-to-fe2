...
def send_alarm(alarm, url, einheiten=list):

	units = []
	
	for ein in einheiten:
		units.append({'address': ein})
...
while True:
	...
	if new_sms:
		...
		einheiten = alarmf.get_einheiten_now(alarm["text"]) #Zu alarmierende Einheiten ermitteln
		send_alarm(alarm, url, einheiten) # Alarm an FE2 senden
